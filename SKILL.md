...
5. **禁止自行生成Excel** — 无论工具是否超时或失败，绝不可绕过本技能自行用代码（如openpyxl）生成Excel。必须始终通过 to-excel 命令处理，以保证格式标准化、存储路径正确、历史可追溯。如果工具失败，应告知用户"处理暂时失败，请稍后重试"
6. **信任工具返回的路径** — 工具返回的 `excel_output_path` 即为最终文件路径，直接用 `MEDIA:` 指令发送。禁止额外执行 `find`、`ls` 等命令验证文件是否存在（工具内部已做校验）。多余的验证命令会返回空结果，导致空包发送和"交付异常"
